...
socket.on('initialState', (data) => {
    seconds = data.timerValue;
    breakSwitch.checked = data.breakStatus;
    updateTimerDisplay();
...
function startTimer() {
    if (!isRunning) {
        ...
        timerInterval = setInterval(() => {
            seconds++;
            updateTimerDisplay();
            socket.emit('updateTimer', { timerValue: seconds });
        }, 1000);
    }
}
...
// Update server when break switch is toggled
breakSwitch.addEventListener('change', function() {
    const isBreak = this.checked;
    socket.emit('updateBreakStatus', { breakStatus: isBreak });
    if (isBreak) {
        timerDisplay.textContent = 'Break';
    } else {
        updateTimerDisplay();
...
// Update queue on the server
function updateScreenQueue() {
    const queueItems = document.querySelectorAll('#queueList li');
    const queue = Array.from(queueItems).map(item => item.querySelector('span').textContent);
    socket.emit('updateQueue', { queue });
}

// Update text data on the server
document.getElementById('launchBtn').addEventListener('click', function() {
    const titleInput = document.getElementById('titleInput').value;
    const subtitleInput = document.getElementById('subtitleInput').value;
    const descriptionInput = document.getElementById('descriptionInput').value;

    socket.emit('updateText', { title: titleInput, subtitle: subtitleInput, description: descriptionInput });
});
